...
class DataHandler:
    def __init__(self, data_root, hyperparam_config, pretrained_model_path=None):
        ...
        self.source_frequency = hyperparam_config.get("source_frequency", 100)
        self.target_frequency = hyperparam_config.get("target_frequency", 100)
...
class LoadData(torch.utils.data.Dataset):
    def __init__(self, root, partitions, speed_conditions, task_conditions, window_size, data_type, dataset_proportion=None, input_mean=None, input_std=None, label_mean=None, label_std=None, normalize = True, source_frequency=100, target_frequency=100):
        ...
        for subject_num, subject in enumerate(partitions):  # Multiple partitions
            ...
            for task in os.listdir(os.path.join(root, subject)):
                ...
                def process_trial_data(input_file_dir, label_file_dir, print_info):
                    ...
                    input_buffer = self.downsample_sequence(input_buffer)
                    label_buffer = self.downsample_sequence(label_buffer)

                    print(f'downsampled input buffer shape is this : {input_buffer.shape}')
                    print(f'downsampled label buffer shape is this : {label_buffer.shape}')
    ...
    def downsample_sequence(self, data):
        if self.target_frequency == self.source_frequency:
            return data

        if self.target_frequency <= 0 or self.source_frequency <= 0:
            raise ValueError("source_frequency and target_frequency must be positive")

        ratio = Fraction(str(self.target_frequency / self.source_frequency)).limit_denominator(1000)
        upsample = ratio.numerator
        downsample = ratio.denominator

        if self.target_frequency > self.source_frequency:
            raise ValueError(
                f"target_frequency ({self.target_frequency}) must be <= source_frequency ({self.source_frequency})"
            )

        if data.ndim == 1:
            return signal.resample_poly(data, upsample, downsample)

        downsampled_channels = [signal.resample_poly(data[:, i], upsample, downsample) for i in range(data.shape[1])]
